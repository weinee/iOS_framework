...
@implementation SDPhotoBrowser 
{
    ...
    BOOL _willDisappear;
		UIStatusBarStyle _originStatusBarStayle;
}
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        self.backgroundColor = SDPhotoBrowserBackgrounColor;
				_originStatusBarStayle = [UIApplication sharedApplication].statusBarStyle;
    }
    return self;
}
...
- (void)photoClick:(UITapGestureRecognizer *)recognizer
{
    ...
    UIView *sourceView = self.sourceImagesContainerViewSubImageViews[currentIndex];
    ...
    [UIView animateWithDuration:SDPhotoBrowserHideImageAnimationDuration animations:^{
        ...
        [self removeFromSuperview];
				[UIApplication sharedApplication].statusBarStyle = _originStatusBarStayle;
    }];
}
...
- (void)showFirstImage
{
    UIView *sourceView = self.sourceImagesContainerViewSubImageViews[self.currentImageIndex];
...
-(NSArray *)sourceImagesContainerViewSubImageViews{
	if (_sourceImagesContainerViewSubImageViews) {
		return _sourceImagesContainerViewSubImageViews;
	}
	NSMutableArray *marr = [NSMutableArray array];
	if (self.sourceImagesContainerView.subviews.count > 0) {
		for (UIView *subView in self.sourceImagesContainerView.subviews) {
			if ([subView isKindOfClass:[UIImageView class]]) {
				[marr addObject:subView];
			}
		}
	}
	_sourceImagesContainerViewSubImageViews = [marr copy];
	return _sourceImagesContainerViewSubImageViews;
}
