...
//  [btnLeft setImage:[UIImage imageNamed:@"icon-back"] forState:UIControlStateNormal];
//  [btnLeft addTarget:self action:@selector(navLeftBtn_Click) forControlEvents:UIControlEventTouchUpInside];
...
- (void)cell_btn_Click:(UIButton *)btn
{
    if (_arraySelectPhotos.count >= self.maxSelectCount
        && btn.selected == NO) {
        ShowToastLong(@"一次最多只能选择%ld张图片，可多次选择", (long)self.maxSelectCount);
        return;
    }
...
- (IBAction)btnDone_Click:(id)sender
{
  ...
  BOOL sucess = [self.navigationController popViewControllerAnimated:YES];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    size.width *= 4;
    size.height *= 4;
    [[ZLPhotoTool sharePhotoTool] requestImageForAsset:asset size:size resizeMode:PHImageRequestOptionsResizeModeNone completion:^(UIImage *image) {
