...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (!self.activeLink) {
			if (self.allowNoLink) {
				if (self.delegate && [self.delegate respondsToSelector:@selector(didClickBeginNoLink:)]) {
					[self.delegate didClickBeginNoLink:self];
				}
			} else{
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    if (self.activeLink) {
        ...
        [self performSelector:@selector(setActiveLink:) withObject:nil afterDelay:self.activeLinkToNilDelay];
				...
				if (self.allowNoLink) {
					if (self.delegate && [self.delegate respondsToSelector:@selector(didClickEndNoLink:)]) {
						[self.delegate didClickEndNoLink:self];
					}
				} else{
...
- (void)touchesCancelled:(NSSet *)touches withEvent:(UIEvent *)event
{
    if (self.activeLink) {
        self.activeLink = nil;
				...
				if (self.allowNoLink) {
					if (self.delegate && [self.delegate respondsToSelector:@selector(didClickCancelNoLink:)]) {
						[self.delegate didClickCancelNoLink:self];
					}
				} else{
